...
node('atlas2') {
    ...
    try {
        configurations.each { container, config ->
            if(config.pr_gpu_unit_tests.size() > 0) {
                jobs[container] = config.pr_gpu_unit_tests.collectEntries { build ->
...
def get_configuration(yaml_file) {
    ...
    if(config.containsKey('pr_gpu_unit_tests')) {
        pr_gpu_unit_tests = config.pr_gpu_unit_tests.collect({ it.toString() })
    }
    ...
    return ["${name}": [
        ...
        "pr_gpu_unit_tests": pr_gpu_unit_tests,
    ]]
}
