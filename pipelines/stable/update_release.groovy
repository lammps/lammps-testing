...
node('slow') {
    ...
    stage('Checkout') {
        dir('lammps') {
            ...
            lammps_release_tag_full = sh(returnStdout: true, script: 'git describe --tags --abbrev=0 | cut -d_ -f2,3').trim()
        }
    }
    ...
    try {
        ...
        stage('Publish') {
            sshPublisher(publishers: [sshPublisherDesc(configName: 'docs.lammps.org', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: "mv /var/www/lammps/download/tars/release.tar.gz /var/www/lammps/download/tars/lammps-${lammps_release_tag_full}.tar.gz", execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: 'lammps/download/tars/', remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'release.tar.gz')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
        }
    } catch (caughtErr) {
        err = caughtErr
