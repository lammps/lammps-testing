        ...
        stage('Build') {
            ansiColor('xterm') {
                sh(label: "Build test binary on ${container}",
                   script: "${launch_container} \$LAMMPS_TESTING_DIR/scripts/simple/unit_tests/${build_script}")
            }
        }
        ...
        stage('Testing') {
            ansiColor('xterm') {
                sh(label: "Run unit_tests/${test_script} on ${container}",
                   script: "${launch_container} \$LAMMPS_TESTING_DIR/scripts/simple/unit_tests/${test_script}")
            }
        }
    }
