...
def regular_build(build_name, set_github_status=true, run_in_container=true, send_slack=true) {
    ...
    def docker_args = ''
    def project_url = 'https://github.com/lammps/lammps-git-tutorial.git'
    ...
    if (currentBuild.result == 'FAILURE') {
        ...
        if (send_slack) {
            slackSend channel: '#lammps-workshop', color: 'bad', message: "Build <${env.BUILD_URL}|#${env.BUILD_NUMBER}> of ${env.JOB_NAME} failed!" + s.message
        }
    } else {
        if (set_github_status) {
        ...
        if (send_slack) {
            slackSend channel: '#lammps-workshop', color: 'good', message: "Build <${env.BUILD_URL}|#${env.BUILD_NUMBER}> of ${env.JOB_NAME} succeeded!" + s.message
        }
    }
}
